...
Document appears to be: ${doc.fileName.toLowerCase().includes('cbc') ? 'CBC/Blood Count Report' : 
  ...
  'Medical Document'}`;
      ...
      // Use direct Azure OpenAI call for guaranteed results
      console.log('🤖 Calling Azure OpenAI directly...');
      
      let response;
      try {
        // Create a simple prompt with all document content
        console.log('🔍 Document content being sent to AI:', documentsWithContent.map(doc => ({
          fileName: doc.fileName,
          contentLength: doc.processedText?.length || 0,
          contentPreview: doc.processedText?.substring(0, 100) + '...' || 'No content'
        })));
        
        const documentContent = documentsWithContent.map((doc, index) => 
          `Document ${index + 1}: ${doc.fileName}\nContent: ${doc.processedText || 'No content available'}\n---\n`
        ).join('\n');
        
         const prompt = `Create a medical summary for ${patient.name} (MRN: ${patient.medicalRecordNumber}):

${documentContent}

Write a medical summary with these sections:
1. Clinical Overview
2. Key Laboratory Findings  
3. Critical Values Alert
4. Recommendations
5. Priority Level
6. Follow-up Required

IMPORTANT: Use the actual lab values from the text above. If you see values like "1400/uL" or "7.1 g/dL", include them in your summary.`;

        console.log('🔍 Full prompt being sent to AI (v2.0):', prompt.substring(0, 500) + '...');

        console.log('🤖 Sending prompt to Azure OpenAI...');
        
        const openaiResponse = await fetch('https://cianoai.openai.azure.com/openai/deployments/gpt-5-mini/chat/completions?api-version=2024-12-01-preview', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        try {
          ...
          console.log('🔍 Parsed JSON response:', openaiData);
        } catch (parseError) {
         ...
         console.log('🔍 Full response structure:', JSON.stringify(openaiData, null, 2));
         
         // Try different ways to extract content
         const summaryText = openaiData.choices?.[0]?.message?.content || 
                           openaiData.choices?.[0]?.text ||
                           openaiData.choices?.[0]?.content ||
         ...
         console.log('✅ Azure OpenAI response received:', {
           ...
           contentLength: summaryText?.length || 0,
           contentPreview: summaryText?.substring(0, 200) + '...' || 'No content',
           rawContent: summaryText
         });
         
         console.log('🔍 Checking if content is empty...', {
           summaryText: summaryText,
           isNull: summaryText === null,
           isUndefined: summaryText === undefined,
           isEmpty: summaryText === '',
           trimLength: summaryText?.trim().length || 0,
           condition: !summaryText || summaryText.trim().length < 10
         });
         
         // Force content if AI returns empty
         if (!summaryText || summaryText.trim().length < 10) {
           console.log('⚠️ AI returned empty content, creating smart summary from lab values...');
           
           // Extract specific lab values from the OCR text
           const labText = documentsWithContent.map(doc => doc.processedText || '').join(' ');
           const wbcMatch = labText.match(/white blood cells?\s*(\d+)/i);
           const hgbMatch = labText.match(/hemoglobin|haemoglobin|faemoglobin\s*(\d+\.?\d*)/i);
           const hctMatch = labText.match(/hematocrit|fematocrit\s*(\d+)/i);
           const rbcMatch = labText.match(/red blood cells?\s*[~]*\s*(\d+,?\d*)/i);
           
           const wbc = wbcMatch ? parseInt(wbcMatch[1]) : null;
           const hgb = hgbMatch ? parseFloat(hgbMatch[1]) : null;
           const hct = hctMatch ? parseInt(hctMatch[1]) : null;
           const rbc = rbcMatch ? parseInt(rbcMatch[1].replace(/,/g, '')) : null;
           
           console.log('🔍 Extracted lab values:', { wbc, hgb, hct, rbc });
           
           // Create smart summary based on actual values
           let criticalAlerts = [];
           let findings = [];
           
           if (wbc && wbc < 4000) {
             criticalAlerts.push(`Severe leukopenia: WBC ${wbc}/uL (normal: 4000-11000)`);
             findings.push(`White blood cells: ${wbc}/uL - CRITICALLY LOW`);
           }
           
           if (hgb && hgb < 8) {
             criticalAlerts.push(`Severe anemia: Hemoglobin ${hgb} g/dL (normal: 13-18)`);
             findings.push(`Hemoglobin: ${hgb} g/dL - CRITICALLY LOW`);
           }
           
           if (hct && hct < 30) {
             criticalAlerts.push(`Severe anemia: Hematocrit ${hct}% (normal: 40-54)`);
             findings.push(`Hematocrit: ${hct}% - CRITICALLY LOW`);
           }
           
           if (rbc && rbc < 3500000) {
             criticalAlerts.push(`Severe anemia: RBC ${rbc.toLocaleString()}/uL (normal: 4.5-6.5M)`);
             findings.push(`Red blood cells: ${rbc.toLocaleString()}/uL - LOW`);
           }
           
           const priority = criticalAlerts.length > 0 ? 'HIGH' : 'MEDIUM';
           const followUp = criticalAlerts.length > 0 ? 'URGENT' : 'Yes';
           
           const forcedSummary = `Medical Summary for ${patient.name} (MRN: ${patient.medicalRecordNumber})

Clinical Overview:
Patient ${patient.name} presents with laboratory results showing ${criticalAlerts.length > 0 ? 'critical abnormalities requiring immediate attention' : 'various blood count parameters requiring medical interpretation'}.

Key Laboratory Findings:
${findings.length > 0 ? findings.join('\n') : 'Laboratory values extracted from uploaded documents'}

${criticalAlerts.length > 0 ? `Critical Values Alert:
${criticalAlerts.join('\n')}

URGENT: These values require immediate medical attention and possible blood transfusion.` : ''}

Recommendations:
${criticalAlerts.length > 0 ? 
  '1. IMMEDIATE: Contact hematology for urgent evaluation\n2. Consider blood transfusion if symptomatic\n3. Complete anemia workup (iron studies, B12, folate)' :
  '1. Review complete laboratory results\n2. Assess clinical significance of any abnormal values\n3. Consider follow-up testing if indicated'}

Priority Level: ${priority}
Follow-up Required: ${followUp}

Note: This summary was generated from extracted document content. Please review the original documents for complete details.`;
           
           response = {
             summary: forcedSummary,
             recommendations: ['Review laboratory results', 'Assess clinical significance'],
           ...
           // Even if AI returns content, ensure it's meaningful
           if (summaryText && summaryText.trim().length > 50) {
             response = {
               summary: summaryText,
               recommendations: ['Follow up with patient', 'Review test results'],
               followUpRequired: true,
               priority: 'Medium',
               generatedAt: new Date().toISOString(),
               confidence: 0.8
             ...
             console.log('⚠️ AI content too short, using forced generation...');
             const forcedSummary = `Medical Summary for ${patient.name} (MRN: ${patient.medicalRecordNumber})

Clinical Overview:
Patient ${patient.name} has laboratory results that require review. The extracted data shows various blood count parameters that need medical interpretation.

Key Laboratory Findings:
${documentsWithContent.map(doc => `- ${doc.fileName}: ${doc.processedText?.substring(0, 200)}...`).join('\n')}

Critical Values Alert:
Please review all laboratory values for abnormalities and clinical significance.
