...
module.exports = async function (context, req) {
    context.log('=== Gmail OAuth callback function called ===');
...
<body>
    <script>
        ...
        // The redirect URI is the URL that Google redirected TO (this callback URL)
        ...
        const redirectUri = `${detectedProtocol}://${hostWithoutPort}/api/auth/google/callback`;
        context.log(`Final redirect URI: ${redirectUri}`);
        
        // Log all header information for debugging
        context.log('=== Redirect URI Detection (Google) ===');
...
<body>
    ...
    <script>
        ...
        context.log(`Exchanging code for token with redirect URI: ${redirectUri}`);
        const tokenResponse = await Promise.race([
            ...
            new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Token exchange timeout after 30 seconds')), 30000)
            )
        ]).catch(timeoutError => {
            context.log(`Token exchange timeout or error: ${timeoutError.message}`);
            throw timeoutError;
        });
        ...
        if (!tokenResponse.ok) {
            ...
            // Try to extract error details from Google's response
            ...
            // Return detailed error for debugging with redirect URI displayed
            const errorHtml = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        ...
        <p style="font-size: 0.85rem; color: #999; margin-top: 1rem;"><strong>Redirect URI used:</strong><br><code style="background: #f5f5f5; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.8rem; word-break: break-all;">${redirectUri.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></p>
        <p style="font-size: 0.85rem; color: #999; margin-top: 0.5rem;">Please ensure this EXACT redirect URI is registered in your Google OAuth app configuration.</p>
        <p style="font-size: 0.85rem; color: #dc3545; margin-top: 0.5rem;"><strong>Error:</strong> ${errorDetails.replace(/</g, '&lt;').replace(/>/g, '&gt;').substring(0, 200)}</p>
        <p style="margin-top: 1rem; font-size: 0.9rem; color: #888;">This window will close automatically...</p>
    </div>
    <script>
        try {
        ...
        try {
            ...
            context.log(`Token response received, parsing JSON...`);
            tokenData = JSON.parse(responseText);
            context.log(`Token data parsed successfully`);
        } catch (jsonError) {
            context.log(`Failed to parse token response: ${jsonError.message}`);
            throw new Error(`Invalid token response from Google: ${jsonError.message}`);
        }
        ...
        if (!accessToken) {
            context.log('No access token in response');
            context.log(`Token response keys: ${Object.keys(tokenData).join(', ')}`);
            throw new Error('No access token received from Google');
        }
        
        context.log('Token exchange successful, proceeding to get user info');
...
<body>
    <script>
        ...
        context.log('Fetching user info from Google...');
        ...
        if (!userInfoResponse.ok) {
            ...
            context.log(`Failed to get user info from Google: Status ${userInfoResponse.status}`);
            context.log(`Error response: ${errorText}`);
            
            // Check if it's a 401 (unauthorized) - might need userinfo scope
            if (userInfoResponse.status === 401) {
                context.log('ERROR: Access token is invalid or missing userinfo scope');
                context.log('This usually means the OAuth scope did not include userinfo.email');
            }
...
<body>
    <script>
        if (window.opener) {
            window.opener.postMessage({
                type: 'email_error',
                message: 'Failed to retrieve user information from Google. Please try again.'
            }, window.location.origin);
            window.close();
        } else {
        ...
        try {
            ...
            throw new Error(`Invalid user info response from Google: ${jsonError.message}`);
        }
        
        const email = userInfo.email;
        context.log(`Retrieved email from Google: ${email}`);
...
<body>
    <script>
        if (window.opener) {
            window.opener.postMessage({
                type: 'email_error',
                message: 'Could not retrieve email address from Google account.'
            }, window.location.origin);
            window.close();
        } else {
...
<body>
    <script>
        ...
        try {
            ...
            context.log('Checking/creating table...');
            try {
                await Promise.race([
                    tableClient.createTable(),
                    new Promise((_, reject) => 
                        setTimeout(() => reject(new Error('Table creation timeout after 10 seconds')), 10000)
                    )
                ]);
                ...
                if (createError.message.includes('timeout')) {
                    context.log(`Table creation timed out: ${createError.message}`);
                    throw createError;
                }
            ...
            context.log('Saving tokens to storage...');
            await Promise.race([
                tableClient.upsertEntity(entity, 'Replace'),
                new Promise((_, reject) => 
                    setTimeout(() => reject(new Error('Storage save timeout after 10 seconds')), 10000)
                )
            ]);
...
<body>
    ...
    <script>
        ...
        context.log(`Error in Gmail callback: ${error.message}`);
    ...
    <script>
        try {
            if (window.opener && !window.opener.closed) {
                ...
                window.opener.postMessage({
                    type: 'email_error',
                    message: 'Failed to connect Gmail account: ${errorMessage.replace(/'/g, "\\'").replace(/"/g, '&quot;')}'
                }, origin);
        ...
        context.log(`Unhandled error in Gmail callback: ${outerError.message}`);
